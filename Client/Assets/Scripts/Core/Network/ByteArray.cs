...
namespace Network
{
    ...
    /// Wrapper of byte[]
    /// Reset before use it
    /// PACKET MAXIUM LENGTH MUST NOT EXCEED 65535
    /// </summary>
    public class ByteArray
    {
        /// <summary>
        /// Packet Structure
        /// |<--- PacketLen(2 bytes) --->|<--- PacketId(2 bytes) --->|<--- redundency bytes(4 bytes) --->|<--- Packet Data(rest bytes) --->|
        /// </summary>
        ...
        public void WriteHead()
        {
            byte[] b = BitConverter.GetBytes((short)m_WritePos);
            Array.Copy(b, 0, m_byteArray, 0, 2);
            b = BitConverter.GetBytes(m_PacketId);
            Array.Copy(b, 0, m_byteArray, 2, 2);
        }
        ...
        private bool CheckWrite(int length)
        {
            ...
            if (newLength <= NetworkConst.MAX_PACKET_LEN)
            {
                ...
                if (m_Length > NetworkConst.MAX_PACKET_LEN)
                    m_Length = NetworkConst.MAX_PACKET_LEN;
        ...
        {
            return (m_ReadPos + length) <= NetworkConst.MAX_PACKET_LEN;
        }
    }
}
